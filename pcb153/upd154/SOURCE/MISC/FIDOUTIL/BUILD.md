...
*(upd154 copy. `FIDOUTIL.CFG` includes `\pcb153\upd154\SOURCE\H`
instead of `\pcb153\SOURCE\H`.)*

**Built 2026-09-23: 11 objects, 0 errors, 153,674 bytes, sha256 prefix
`b7f01f3885f0f08d…` — byte-identical to the 15.3 build.** That is not a
mistake. FIDOUTIL's own ten sources are byte-identical between the two
trees, and although 33 of the shared headers differ, none of the fields
they add is one FIDOUTIL reads. Same input, same output. It will diverge
the moment FIDOUTIL is taught anything 15.4-specific.

## It did not build until a header bug was fixed

`PASSTHRU.CPP` failed with two bare *"Declaration syntax error"*s at
`USERS.H` 432 and 437 — the `extern personal_psa_t Personal;` and
`extern timebank_psa_t Bank;` lines.

Both typedefs were declared **inside `#ifdef PCB152`**, while the externs
that use them sit outside it. Any translation unit that does not define
`PCB152` — FIDOUTIL is one — lost the types and then hit the
declarations. The error points at the externs and says nothing about the
cause; `CPP.EXE` on the preprocessed output is what showed the typedefs
were simply absent.

They are 15.4 additions and have nothing to do with the 15.2 extended
user info that `PCB152` guards, so they were moved out, unconditional,
above the declarations that use them. Two stray duplicate
`#pragma pack()` lines went with them.
...
or copy it to `TURBOC.CFG` and just run `BCC`:
    ...
    BCC -c -nOBJ \pcb153\upd154\SOURCE\UTIL\PCBSETUP\SOURCE\CI_BUILD.C
    BCC -c -nOBJ \pcb153\upd154\SOURCE\FIDO\DATA.CPP
    BCC -c -nOBJ \pcb153\upd154\SOURCE\FIDO\PASSTHRU.CPP
