...
/* Personal PSA — new in 15.4.  See USERS.H for personal_psa_t layout. */
...
long static PersonalOffset;     /* 15.4 */
long static TotalAppSize;
#endif

/* 15.4: PSA_PERSONAL added before PSA_END */
typedef enum {PSA_NONE=0,PSA_ALIAS,PSA_VERIFY,PSA_ADDRESS,PSA_PASSWORD,PSA_STATS,PSA_NOTES,PSA_ACCOUNT,PSA_QWKNET,PSA_PERSONAL,PSA_END} psadeftype;
    ...
    if (! NeedToUpdate && Header.NumOfApps != 0) {
      ...
      for (X = 0; X < Header.NumOfApps; X++) {
        ...
        if (AppHeader.Version == 10 && memcmp(AppHeader.Name,"PCB",3) == 0) {
          ...
          if (strcmp(&AppHeader.Name[3],"PERSONAL") == 0 && AppHeader.SizeOfRec == sizeof(personal_psa_t)) {
            PersonalSupport = TRUE;
            PersonalOffset  = AppHeader.Offset;
          }
        }
      }
...
long pascal adduserrecord(void) {
  ...
  /* 15.4 fix (per HISTORY):
   *   "Fixed the default 'M' for Gender to be empty until the user
   *    specifies either 'M' or 'F'."
   *
   * Initialize the personal PSA fields to their "unset" defaults so
   * that a new user record doesn't presume a gender.  In 15.3 (pre-fix),
   * PCBoard's registration flow initialized Gender = 'M', causing the
   * user list to show every never-configured user as male.  15.4 uses
   * ' ' (space) to mean "not yet specified".
   */
  if (PersonalSupport) {
    memset(&Personal, 0, sizeof(Personal));
    Personal.Gender = ' ';                          /* 15.4: unset, not 'M' */
  }
