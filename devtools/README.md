...
|---|---|---|
| `TURBOC201.zip` | `devtools/TC201BT.ZIP` (build subset) | Turbo C 2.01 → PCBKIT |
...
| `MSC70-retail.7z` | `devtools/MSC70BT.ZIP` (packaged) | MS C 7.0 retail disks → PCBKMS |
...
### CodeBase (Sequiter Software, LGPL v3.0)
| Archive / location | What it is |
|---|---|
| `pcbcbase/CODEBASE/SOURCE/` | Local copy of CodeBase source (dBASE III+ engine) |
| [CodeBase-for-DBF](https://github.com/MPSystemsServices/CodeBase-for-DBF) | Upstream repo (2.8 GB — too large to mirror) |

PCBoard uses CodeBase for all dBASE/PPL database support.  Two libraries
must be compiled from source — neither is pre-built in the repo:

| Library | Compiler | Target | Expected path |
|---|---|---|---|
| c4base.lib | BC31 (large model) | DOS | `pcbcbase/codebase/bor31/c4base.lib` |
| b4.lib | BCOS2 (flat model) | OS/2 | `pcbcbase/codebase/os2bor1/b4.lib` |

Without c4base.lib, DOS PCBOARD.EXE has no dBASE support compiled in.
Without b4.lib, OS/2 PCBOARD2.EXE cannot link at all.
Build script: `BLDCBASE.BAT` (planned).
