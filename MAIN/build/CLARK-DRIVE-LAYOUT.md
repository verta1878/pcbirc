# Clark's drive layout — what `\PROJ`, `\BC31`, `\TA`, `\LIBS` are
...
| `386MAX_S\`, `D32A\`, `HX\`, `CWSDPMI\` | DPMI hosts |
...
|---|---|---|
| `\PROJ` | every utility `.MAK`: `ROOT = \PROJ` | the project root — `$(ROOT)\LIB`, `$(ROOT)\PCB\SOURCE\H`, `$(ROOT)\PCBSM\SOURCE`, `$(ROOT)\PCBSETUP\SOURCE` |
| `\BC31` | `MISC\BCDOS.BAT`: `BCROOT=\BC31` | Borland C++ 3.1 |
| `\TA` | `MISC\BCDOS.BAT`: `ASMROOT=\TA` | Turbo Assembler — `\TA\TASM.EXE` |
| `\LIBS` | `MISC\BCDOS.BAT`: `LIBSDIR=\LIBS` | CodeBase, and `\LIBS\VMDATA\BC31_DOS\VMDATA.LIB` |
...
The category libraries are referenced per memory model: `DOS_L.LIB`,
`DOS_S.LIB`, `DOS_C.LIB`. The repo only has the **large** set, so the
small-model utilities fail with:

    Fatal: '\proj\lib\bcdos\bc31\dos_s.lib' does not exist
...
`\LIBS\VMDATA\BC31_DOS\VMDATA.LIB` and `vmdata.h` — the virtual memory
...
| `\LIBS\VMDATA\BC31_DOS\VMDATA.LIB` + `vmdata.h` | virtual memory library | source in `toolkit/pwa153/{SOURCE,H}` (the crew's clean-room version); **the .LIB is not built yet** |
...
| Copy from | To |
|---|---|
| `toolkit/pwa153/{SOURCE,H,CFG}` | `\PROJ\LIB\{SOURCE,H,CFG}` |
| `toolkit/pwa153/bc31/lib/*.LIB` | `\PROJ\LIB\BCDOS\BC31\` |
| `pcb153/SOURCE` | `\PROJ\PCB\SOURCE` |
| `PCBSRCV/000/UTIL/PCBSM` | `\PROJ\PCBSM` |
| `PCBSRCV/000/UTIL/PCBSETUP` | `\PROJ\PCBSETUP` |
| `PCBSRCV/000/UTIL/<x>` | `\PROJ\UTIL\<x>` |
| `PCBSRCV/000/MISC/<x>` | `\PROJ\MISC\<x>` |
| `BC31\BIN\TASM.EXE` | `\TA\TASM.EXE` |
| `pcbcbase/CODEBASE` | `\LIBS\CODEBASE` |
| COMMDRV libs + headers | `\LIBS\COMMDRV\{LIB,H}` |
| *(missing)* | `\LIBS\VMDATA\BC31_DOS\` |
...
- **`\PROJ\LIB` must hold the category libraries per memory model**
  (`DOS_L.LIB`, `DOS_S.LIB`, `DOS_C.LIB`). Only the large set exists —
...
`pcb154/normalize_case.sh`, which covered three directories in `pcb154`
