...
*             NULL is encountered in the file prior to MaxChars being
*             reached
*
*  Returns :  -1 on error
*             0  if the Str was NULL delimited
*             1  if either the MaxChars was hit or no NULL was found
*/

int LIBENTRY dosfngets(char *Str, unsigned MaxChars, DOSFILE *file) {
  char *ptr_to_head;
  char *ptr_to_null;
  unsigned BytesRead;
  unsigned OldOffset;
  ...
  while (1) {
    ...
    if ((ptr_to_null = strnchr(ptr_to_head,0,BytesRead)) != NULL) {
      BytesRead = (unsigned) (ptr_to_null - ptr_to_head);
      if (BytesRead >= MaxChars) {
        ...
        file->offset++;                     /* get rid of NULL by adding 1 */
        file->curpos += file->offset - OldOffset;
      }
