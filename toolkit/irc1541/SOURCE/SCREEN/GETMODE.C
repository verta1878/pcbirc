...
void near pascal getvideotype(void) {
  ...
  _asm mov ax,1A00h      /* Attempt to call VGA Identify Adapter Function */
  _asm int 10h
  _asm mov ALreg,Al
  _asm mov BLreg,Bl
  ...
  if (ALreg == 0x1A) {    /* if AL changed from 0 to 0x1A - must be PS/2 bios */
    ...
    _asm mov ah,12h      /* select Alternate Function service */
    _asm mov bx,10h      /* BL=0x10 means return EGA information */
    _asm int 10h
    _asm mov BXreg,Bx
    ...
    if (BXreg != 0x10) {  /* BX changed means there's an EGA there */
      ...
      _asm int 11h       /* we got this far, must be CGA or MDA */
      _asm and al,30h
      _asm shr al,1
      _asm shr al,1
      _asm shr al,1
