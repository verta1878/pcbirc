...
void pascal adjustacct(void) {
   ...
   *   "If a PCBoard PSA wasn't installed, you were still allowed to edit
   *    the information for the PSA.  Fixed."
   *
   * Gate the Account-balance adjustment editor on the AccountSupport flag.
   * If the Account PSA hasn't been installed via PCBSetup, the sysop's
   * input has nowhere to be stored — pre-15.4 PCBSM would silently accept
   * their entries and then drop them on the floor.  Now we show a clear
   * "PSA not installed" message and abort.
   */
  extern bool AccountSupport;                       /* from USERS.C */
  if (!AccountSupport) {
    clsbox(1,1,78,23,Colors[OUTBOX]);
    fastprintmove(19,11,"PSA not installed",Colors[DESC]);
    fastprintmove(9,13,"The Account (banking) PSA has not been installed on this system.",Colors[DISPLAY]);
    fastprintmove(3,15,"Run PCBSetup and install the Account PSA before running this option.",Colors[DISPLAY]);
