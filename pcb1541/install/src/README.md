...
| v1.10.2 | ? | ? | Variables + control flow + string ops (`@If`/`@Endif`/`@Else`/`@Goto`/`@Label`/`@Set`/@Str*). Needed to make real INSTALL.DAT run beyond the first branch. |
| v1.10.3 | ? | ? | Filesystem + disk sequencing (`@MkDir`/`@ChDir`/`@Exists`/`@DefineDisk`/`@GetOutDrive`/`@GetSubdir`). |
| v1.10.4 | ? | ? | Interactive menu (`@GetGroups`/`@CheckBox`/`@Group`/`@AskOverwrite`). |
| v1.10.5 | ? | ? | System hooks (`@System`/`@SetAutoexec`/`@SetConfig`/`@Finish`). End-to-end run against real INSTALL.DAT. |
