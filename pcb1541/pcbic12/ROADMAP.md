...
- **pcbic v1.0.2** — TESTIC.EXE understood (disassembly pass, C source
  exported).
...
| 3 | PCBICEVT.EXE |  89,612 | v1.0.4 | Event handler. Moderate. |
| 4 | PCBICCFG.EXE | 185,398 | v1.0.5 | Config UI. Big — lots of screen drawing code. |
| 5 | Pcbic.exe    | 313,310 | v1.0.6 | Main IC binary. Huge — TCP/IP stack integration, all 10 menu commands. |
| 6 | Pcbic2.exe   | 217,111 | v1.0.7 | OS/2 sibling of Pcbic.exe. |

### Per-EXE workflow (off-sandbox disassembly)

For each target:

1. **Import** into the disassembler, auto-analyze
...
4. **Function-by-function decompile → C source**
   - disassembler output as starting point
...
6. **Byte-diff**
   - `cmp -l <built binary> bin/<original>`
...
Only compilable source lands in `src/`. Working notes, per-binary
analysis, and raw decompiler exports stay local (never tracked).
Build outputs land wherever the local build puts them; nothing to
track.
...
**Acceptance per binary**: `cmp -s <built binary> bin/<original>`.
...
2. the disassembler the real INSTALL.EXE to confirm @-command semantics
...
## What the disassembler work looks like from Claude's side

Claude does not run the disassembler directly (sandbox has no the disassembler + no X
server for the GUI). The workflow:

- **You** (on your Linux box): run the disassembler, work through analysis,
  export decompiled C locally, then commit only cleaned-up
  `*.c` files to `src/<binary>/`
- **Claude**: reviews the exports, spots Borland RTL patterns the disassembler
...
- `pcb1541/pcbic12/src/<binary>/` — the disassembler-driven reverse-eng work,
...
3. **TESTIC.EXE** (v1.0.2) — first the disassembler target, warm-up before
   the big EXEs
4. **TESTIC2.EXE** (v1.0.3) — OS/2 sibling, isolates DOS/OS2 delta
5. **PCBICEVT.EXE** (v1.0.4) — moderate complexity, event handler
   patterns are common enough to make progress steadily
6. **PCBICCFG.EXE** → **Pcbic.exe** → **Pcbic2.exe** (v1.0.5, v1.0.6, v1.0.7)
   — accepting these are multi-month efforts each

---

*pcbic v1.0.0 opens the arc — reference in `bin/`, source targets defined,
tooling in place. v1.0.1 starts the burn-down; v1.1 = whole arc complete.*
